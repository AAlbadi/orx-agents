...
def _safe_get_msg_role(m: Any) -> str:
    """Safely extracts role from dict, LLMSpecificMessage, or Content object."""
...
def _safe_get_msg_content(m: Any) -> str:
    """Safely extracts text content from dict, LLMSpecificMessage, or Content object."""
    if hasattr(m, "message"):
        m = m.message
    if isinstance(m, dict):
        ...
        if not content and "parts" in m:
            parts = m.get("parts", [])
            return "".join(p.get("text", "") if isinstance(p, dict) else getattr(p, "text", "") for p in parts)
        ...
        try:
            ...
            llm = GoogleLLMService(
                ...
                settings=GoogleLLMService.Settings(
                    ...
                    temperature=0.3,
                    max_tokens=150,
                    thinking=GoogleLLMService.ThinkingConfig(thinking_level="minimal"),
                ),
            )
        except Exception as e:
                ...
                if isinstance(frame, LLMRunFrame):
                    ...
                    if frame.context and hasattr(frame.context, "get_messages"):
                        ...
                        for m in reversed(msgs):
                            if _safe_get_msg_role(m) == "user":
                                user_text = _safe_get_msg_content(m)
                                break
        ...
        async def process_frame(self, frame: Frame, direction: FrameDirection):
            ...
            if isinstance(frame, LLMContextFrame):
                ...
                if frame.context and hasattr(frame.context, "get_messages"):
                    ...
                    if len(all_msgs) > 7:
                        sys_msgs = [m for m in all_msgs if _safe_get_msg_role(m) == "system"]
                        conv_msgs = [m for m in all_msgs if _safe_get_msg_role(m) != "system"][-6:]
